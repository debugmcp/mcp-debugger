import { setInitialized, setAdapterConfigured, setCurrentThreadId, getPendingRequest, removePendingRequest } from './state.js';
/**
 * Main handler for proxy messages
 */
export function handleProxyMessage(state, message) {
    // Validate session ID
 ...
 */
function handleStatusMessage(state, message) {
    const commands = [];
    switch (message.status) {
        case 'proxy_minimal_ran_ipc_test':
            commands.push({ type: 'log', level: 'info', message: '[ProxyManager] IPC test message received' }, { type: 'killProcess' });
            break;
        case 'dry_run_complete':
            commands.push({ type: 'log', level: 'info', message: '[ProxyManager] Dry run complete' }, { type: 'emitEvent', event: 'dry-run-complete', args: [message.command, message.script] });
            break;
        case 'adapter_configured_and_launched':
            commands.push({ type: 'log', level: 'info', message: '[ProxyManager] Adapter configured and launched' }, { type: 'emitEvent', event: 'adapter-configured', args: [] });
            // Update state
        ...
        case 'terminated':
            commands.push({ type: 'log', level: 'info', message: `[ProxyManager] Status: ${message.status}` }, {
                type: 'emitEvent',
                event: 'exit',
 ...
 */
function handleErrorMessage(state, message) {
    return {
        commands: [
            {
                type: 'log',
 ...
 */
function handleDapEvent(state, message) {
    const commands = [
        {
            type: 'log',
            level: 'info',
 ...
 */
function handleDapResponse(state, message) {
 ...
 */
export function isValidProxyMessage(message) {
