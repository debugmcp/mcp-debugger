 ...
 * drift between them.
 */
import { BREAKPOINT_STOP_REASONS, isTerminalSessionState, SessionState } from '@debugmcp/shared';
...
/**
 * Stops only a live debugger produces. A `stopped` with one of these reasons
 * clears the decision: the adapter build debugs after all. `pause` is not
 * among them — js-debug lands a user pause under `noDebug` with its debug
 * domains (breakpoints) still off, so a pause proves nothing about binding.
 */
 ...
 * launch is still running or paused. Once the session is stopped or in
 * error the record describes nothing that is running — a breakpoint set
 * then is an ordinary queued one for the next launch.
 */
export function isDebuggerOff(session: DebuggerOffView): boolean {
  return session.debuggerDisabled === true && !isTerminalSessionState(session.state);
}
