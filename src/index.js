...
(() => {
    ...
    const hasStdio = process.argv.some(arg => arg === 'stdio' ||
        arg === '"stdio"' ||
        arg === '"\'stdio\'"' ||
        arg.includes('stdio'));
    ...
    const hasTransportArg = process.argv.some(arg => arg === '--transport' || arg.includes('transport'));
...
process.argv = process.argv.map(arg => typeof arg === 'string' ? arg.replace(/^["'](.*)["']$/, '$1') : arg);
...
export function createDebugMcpServer(options) {
...
export async function main() {
    ...
    // Setup commands
    setupStdioCommand(program, (options) => handleStdioCommand(options, { logger, serverFactory: createDebugMcpServer }));
    setupSSECommand(program, (options) => handleSSECommand(options, { logger, serverFactory: createDebugMcpServer }));
