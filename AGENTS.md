...
- `packages/` hosts 18 workspace modules. Thirteen carry their own `src/` trees (`@debugmcp/shared`, `adapter-mock`, `adapter-python`, `adapter-ruby`, `adapter-javascript`, `adapter-rust`, `adapter-go`, `adapter-java`, `adapter-dotnet`, `adapter-cpp`, `adapter-cobol`, `codelldb-common`, `mcp-debugger`); the other five — `codelldb-win32-x64`, `codelldb-linux-x64`, `codelldb-linux-arm64`, `codelldb-darwin-x64`, `codelldb-darwin-arm64` — are prebuilt CodeLLDB payloads with no TypeScript in them. A `packages/logs/` may linger in a checkout that ran pre-#637 builds (server logs now default to `os.tmpdir()/debug-mcp-server/`); it is not a package and not in the repository — `.gitignore`'s `logs/` rule covers it.
