...
- **`step_over` on an out-of-line `PERFORM` runs the performed range** and stops on the statement after the PERFORM (`PERFORM 1000-INIT` → the next line of `0000-MAIN`, with `1000-INIT`'s effects visible), `PERFORM … TIMES`/`UNTIL`/`VARYING` loops included — one stop after all iterations. The shim reads libcob's PERFORM stack (`frame_ptr - frame_stack` in the program's frame) and runs to two temporary stops: a breakpoint on the next statement in cobc's statement order (across a `COPY` of procedure statements) and, inside a performed range, an instruction breakpoint on the range's own return address, so a PERFORM that is the last statement of a performed paragraph steps out to the performer. `step_into` on a PERFORM enters the paragraph. An inline `PERFORM … END-PERFORM` steps into its body like a loop. A `GO TO` out of the range escapes both stops: the program runs to the next user breakpoint — set one after the PERFORM in that case. The stop's description says what happened (`stepped over the PERFORM at hello.cob:32`).
- **`step_out` inside a performed paragraph** runs to the range's return and on to the statement after the PERFORM that entered it (`returned from 1000-INIT to 0000-MAIN`); a `PERFORM … TIMES` re-entering the paragraph is run through to the statement after the PERFORM. Outside any PERFORM, `step_out` leaves the program as before.
...
- **Logpoints**: `set_breakpoint {file, line, logMessage: "total={WS-TOTAL} idx={WS-IDX}"}`. The `{…}` parts are COBOL data references evaluated the way `evaluate_expression` does (qualification, subscripts, reference modification, 88-levels as `true`/`false`; `{/nat expr}` for a C expression), rendered into `get_output` (console category) each time the line is reached, and the program resumes without a client-visible stop; an unknown name renders as `<unavailable: …>` inline. The `logMessage` never reaches CodeLLDB — its own `{…}` interpolation aborts the adapter on a COBOL name (measured on 1.11.8). A logpoint on a line that also has a pausing breakpoint logs and pauses; one met during a step logs and the step goes on.

## Not supported yet

| Feature | Status |
|---|---|
| Synthesised PERFORM stack in `get_stack_trace` (the performing paragraphs as frames) | Later milestone; the PERFORM depth is readable as `/nat (long)(frame_ptr - frame_stack)` |
