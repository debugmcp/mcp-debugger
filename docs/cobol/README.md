...
- On Windows the attach stop is reported on the break thread the OS injects (exception `0x80000003`), a pause may land on a runtime worker thread: when the reported thread has no COBOL frame, the shim reports the stop on the thread that is inside the COBOL program (the description names both), so `get_stack_trace` shows the program on first contact. Breakpoint, step and runtime-error stops are always on the right thread and are left alone.
...
- Measured (M2, Windows/MSYS2 3.2): attaching to `examples/cobol/pause.cob` by PID lands inside the OS sleep under libcob's `C$SLEEP`; `get_stack_trace` shows `PAUSE: 0000-MAIN`, `get_local_variables` reads `WS-TICK` through the walk-up, `evaluate_expression WS-TICK` answers `3 (evaluated in frame #6 PAUSE: 0000-MAIN …)`, and `detach_from_process` leaves the job ticking. It goes through the same CodeLLDB attach the C/C++ adapter uses, plus the shim.
...
- **`step_out` of the outermost program** stops in the generated C `main` (there is no COBOL statement left to reach); `continue` from there runs the program to its exit.
