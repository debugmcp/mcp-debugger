...
On Windows, prebuilt executables and modules need DWARF for COBOL variable addresses.
The adapter rejects a verified PDB-only binary with rebuild guidance (`cobc -g -A "-O0 -gdwarf-4"`).
DWARF and mixed DWARF/PDB binaries are accepted. Unreadable, stripped or unrecognized
files produce a diagnostic and continue; a sibling `.pdb` alone does not cause rejection.
...
- When `sources` is supplied, missing cobc or a failed translation fails launch before the engine starts, unless `manifestDirs` contains usable manifests from an earlier build. Missing directories, malformed JSON and empty manifests do not provide a fallback. Omit `sources` to explicitly use the C view without a compiler.
