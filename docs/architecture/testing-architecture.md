...
- `MockProxyManager`: `simulateStopped(threadId, reason)`, `simulateEvent(event, ...args)`, `simulateError(error)`, `simulateExited(exitCode)`, `simulateExit(code, signal)`
