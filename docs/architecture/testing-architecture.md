...
**`createMockDapClient()`** (`dap-client.ts`) — an `IDapClient` double over a real `EventEmitter`, every method a `vi.fn` (the `MockDapClient` type), shared by the proxy-worker, go-initialized-fallback and connection-manager tests. Program responses with `mockDapClient.sendRequest.mockResolvedValue({ body: { ... } })` — or `.mockImplementation(async (command) => ...)` for per-command answers — and failures with `sendRequest.mockRejectedValue(error)` / `connect.mockRejectedValueOnce(error)`. DAP events (`initialized`, `stopped`, `continued`, `exited`, `terminated`, `output`, `breakpoint`, etc.) are emitted directly on it: `mockDapClient.emit('stopped', body)`.

**`createMockLogger()` / `createMockFileSystem()` / `createMockProcessSpawner()`** (`dap-proxy-doubles.ts`) — the rest of the proxy worker's dependency slice, typed to the worker-local `ILogger`/`IFileSystem`/`IProcessSpawner` in `src/proxy/dap-proxy-interfaces.ts` rather than `@debugmcp/shared`'s wider interfaces (the worker declares the narrower slice it uses, and a double typed to the wide interface would hide a member the worker starts calling). Shared by the proxy-worker and go-initialized-fallback tests alongside `createMockDapClient()`.
