...
- **COBOL** (issue #759): the image installs the `gnucobol3` package, so a `.cob`/`.cbl` launch compiles in-container (into `.debug-mcp/cobol/` beside the source) and the COBOL adapter is preloaded; the same Linux-binary rule applies to prebuilt COBOL executables. See [docs/cobol/README.md](cobol/README.md).
