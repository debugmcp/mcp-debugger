...
| Java, .NET, Ruby, COBOL | Not supported — `set_breakpoint` with `logMessage` fails fast with a clear error (rdbg would silently convert a logpoint into a *pausing* breakpoint, issue #469; the COBOL shim does not interpolate `{WS-NAME}` yet, issue #759) |
