...
- `dapLaunchArgs` (object, optional): Standard DAP launch arguments:
  ...
  - `noDebug` (boolean): Run the program with the debugger disabled (the standard DAP flag; default `false`). Every adapter honours it: no breakpoint binds, no exception filter arms, no entry stop lands, and no stop ever arrives. Where the adapter still completes the launch handshake (js-debug does), a short script ends `stopped` and a server stays `running`; debugpy sends no `initialized` event without a debugger and the launch currently fails init instead (issue #746). A bare `noDebug` run is legitimate and says nothing; when the session has breakpoints (line or function), an explicit `breakOnExceptions` other than `"none"`, or `stopOnEntry`, the response `warning` says the debugger is off and names what will not fire, in place of the unbound-breakpoint diagnostics that would otherwise send you to check paths that are fine (issue #710). The same warning comes back from `restart_debugging` and from a `dryRunSpawn` check.
