...
**`debuggerDisabled`:** present (`true`) while the current launch runs with the debugger off — `dapLaunchArgs.noDebug: true` on an adapter that honours it (see `start_debugging`). Cleared by the next launch or attach, and by any `stopped` event that arrives anyway (issue #749).
