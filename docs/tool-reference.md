...
**Parameters:**
- `language` (string, required): The programming language to debug. Languages are discovered dynamically from installed adapters. The default fallback languages (when dynamic discovery is unavailable) are `"python"` and `"mock"`. When all adapters are available, the full list is: `"python"`, `"ruby"`, `"javascript"`, `"rust"`, `"go"`, `"java"`, `"dotnet"`, `"cpp"`, `"cobol"`, `"mock"`. The actual list depends on which `@debugmcp/adapter-*` packages are discoverable at runtime. A language may be usable in one mode only — e.g. in the Docker container Ruby is attach-only (the adapter ships without a Ruby runtime; attach connects directly to a remote rdbg socket). Check `list_supported_languages` `modes` for per-mode availability; creating a session for an attach-only language is allowed, and only `start_debugging` will fail.
...
- `function` (string, optional): **Symbol addressing** — break on entry to a function/method by name (DAP function breakpoint). Session-global: no `file` or `line` at all, and names survive edits better than both. Composes with `condition` only. Supported by Python, Go, Rust, C/C++, .NET, Java, JavaScript and COBOL (JavaScript names are dotted runtime paths delivered over the CDP bridge — see [Function breakpoints](#function-breakpoints); COBOL names are paragraphs, sections or a PROGRAM-ID, qualifiable as `1000-INIT OF PAYROLL`, resolved by the COBOL shim to the range's first statement); Ruby is rejected up front (rdbg advertises the capability but its handler ignores the request, so nothing would ever bind).
...
- Support is adapter-gated: Python, Go, Rust, C/C++, .NET, Java, JavaScript and COBOL (paragraph, section and PROGRAM-ID names) work; Ruby is rejected up front — rdbg's initialize response advertises `supportsFunctionBreakpoints`, but the debug gem's DAP handler acknowledges the request and ignores it (verified against debug 1.11.0), so the Ruby policy pins the capability off rather than trust the advertisement.
...
| Adapters | Behavior |
|---|---|
| Python, JavaScript/TypeScript, Go, Rust, C/C++, COBOL, mock | Supported — logs without pausing (COBOL: `{WS-NAME}` is a COBOL data reference evaluated by the shim) |
...
```json
{
  ...
  "data": {
    "message": "Paused",
    "stopReason": "pause",
    "location": { "file": "/path/app.py", "line": 42, "column": 1 }  // the first visible frame; if the pause landed inside a hidden internal frame, get_stack_trace's pausedFrame names it
  }
}
```
...
- `frameId` (number, optional): Authoritative stack frame ID for context. If omitted, evaluation uses the same adopted top frame as `get_stack_trace` and `get_local_variables` — at a breakpoint or step that is the frame the program stopped in even when the default stack display would hide it as internal (issue #672). `anchorNote` explains an automatic thread switch and any paused-frame decision (kept or hidden), and `frame` names the frame that was actually used — on failure too, so an `Unable to evaluate on async stack frame` error says which frame it tried. Compare `frame` with `get_stack_trace`'s `pausedFrame` to tell whether the evaluation ran in the frame the program stopped in.
