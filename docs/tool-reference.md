...
- Support is adapter-gated: Python, Go, Rust, .NET, and Java work; JavaScript (js-debug does not implement function breakpoints upstream) fails fast with a clear error; Ruby is accepted with a warning and validated against the adapter's live capabilities at launch.
