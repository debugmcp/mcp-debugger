...
- [ ] **CodeLLDB bump procedure** (when bumping the CodeLLDB pin): update `vendor-manifest.json` (version + all five VSIX digests), bump the five `packages/codelldb-<platform>/package.json` versions to match, update `DEFAULT_CODELLDB_VERSION` in `packages/codelldb-common/src/codelldb-resolver.ts`, re-vendor, and run the rust + cpp + codelldb-common test suites (the drift-guard tests enforce all three stay in sync). The next release then republishes exactly the five platform packages; at an unchanged pin the npm-view guards skip them.
