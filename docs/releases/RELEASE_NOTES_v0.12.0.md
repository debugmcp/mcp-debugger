...
> **Note on path handling:** In host mode, `SimpleFileChecker` rejects relative paths outright (they are not resolved). In container mode, all paths not already starting with the workspace root (`MCP_WORKSPACE_ROOT`, which has no default and must be set explicitly) are rewritten to fall under it -- this includes relative paths (resolved against the workspace root) and non-workspace absolute paths (e.g., `/home/user/test.py`). After validation, the server passes the resolved effective path (not the original raw path) to the session manager.
