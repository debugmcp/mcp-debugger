...
### Added
- **Dev-proxy restarts accept persistent backend environment overrides** — `dev_restart_debugger` and `dev_rebuild_and_restart` can replace, preserve, or clear diagnostic environment settings without restarting the MCP client; status reports display-safe values using the shared secret redactors, failing closed on an unbuilt checkout (#527)
- **Deterministic adversarial DAP fixture** — resilience tests can drive the production proxy worker and TCP framing stack with JSON scenarios that delay or drop responses, reorder events, inject junk, duplicate frames, close at controlled stages, or never listen; the proxy initialization deadline is injectable for these tests while production retains its 30-second default (#511)
- **Case study: the initialize response that never came** — [docs/case-studies/rdbg-initialize-response-stall.md](docs/case-studies/rdbg-initialize-response-stall.md) walks the full self-debugging RCA of the #492 Ruby launch stall: mcp-debugger stepping through its own compiled DAP client with statement breakpoints, the fake-rdbg replay, the rdbg 1.11.0 source analysis, and what #492/#493 changed; troubleshooting and ruby docs link the relevant symptoms to it
...
### Fixed
- **Dev-proxy lifecycle cleanup is quiet, serialized, and container-safe** — intentional Streamable HTTP shutdown no longer reports an SSE `AbortError` or orphan warning; automatic startup cannot race an immediate restart into duplicate backend clients; and custom `docker run` backends receive an ownership label so stdio containers are removed during restart instead of accumulating. Status metadata also describes the fields it actually returns (#498, #536, #539, #541)
- **Transport and worker diagnostics reach the right log level and file** — the deprecated SSE command now attaches `--log-file` to its CLI logger before emitting lifecycle lines, matching HTTP, while proxy-worker stderr maps `[DEBUG]`, `[INFO]`, `[WARN]`/`[WARNING]`, and `[ERROR]` prefixes to their corresponding server-log levels; indented continuations inherit their diagnostic's level and unprefixed crash output remains an error (#533, #534)
- **Ruby attach honors explicit shared-filesystem configuration** — `adapterConfig.localfs` was advertised but silently overwritten from the attach host, preventing a containerized mcp-debugger from binding target-side paths even when the source was mounted identically; explicit booleans now win while absent or invalid values retain host inference, and the Ruby guide documents the correct `adapterConfig` placement plus rdbg's Windows drive-colon mapping limitation (#499)
- **Windows accepts an extensionless Ruby script in `RDBG_PATH`** — when `RDBG_PATH` resolved directly to the gem-installed `rdbg` script instead of its `.bat`/`.cmd` shim, Node.js tried to spawn the extensionless file and failed with `ENOENT`; mcp-debugger now launches that script through the configured Ruby interpreter while preserving executable and non-Windows behavior (#508)
- **js attach no longer burns 12s polling `threads` for a stop it must not force** — every js attach ran the worker's `ensureInitialStop` poll (~120 `threads` round-trips over 12s) even though js-debug's child reports its sole thread as id 0 — which the poll's `> 0` guard can never accept — and even though the session manager already owns the post-attach pause per-policy; the run always ended in a false `no threads discovered within timeout` warning. Attach now skips the worker-side entry-stop entirely, launch runs it only when `stopOnEntry` was actually requested, and a threads report whose first id is 0 counts as discovery (logged, no pause) instead of polling into a warning that contradicts the trace (#520)
- **`pause_execution` actually pauses an attached idle Node server** — on a js attach session the pause was accepted but the stop never landed, no matter how much JavaScript the target later ran: js-debug's smart-stepper treats a user pause that lands on a blackboxed/unmapped frame (on a mostly-idle server: every frame) as something to auto-step past, and its >256-step failsafe switches to step-out, which never escapes an idle event loop — an endless step-chase at ~40ms per CDP round-trip, observable as a flood of `continued` events with no `stopped`. js attach now defaults `smartStep: false` (caller-overridable via `adapterConfig`, launch unchanged; breakpoint/exception/entry stops were always exempt in js-debug itself). Two routing holes the investigation surfaced are also closed: a pause dispatched before js-debug's child target session finished its attach handshake was swallowed without a response (child-required commands now wait for adoption to *complete*, not merely for a child connection to exist), and a pause issued when no child session can ever appear used to fall through to the js-debug root session — which acks `pause` as a silent no-op — and now fails with an actionable "no debug target" error instead of a forever-`pending` success (#513)
- **DAP trace records name the connection that produced them** — with `DAP_TRACE=1`, all of a proxy worker's DAP clients (the parent adapter connection, js-debug's adopted child target, throwaway release connections) append to one `dap-trace-<sessionId>.ndjson` with independent `seq` spaces, so two `initialize` requests with `seq: 1` from different sockets were byte-identical in the file — the #513 investigation had to reconstruct "did this pause go to the root or the target session?" by joining wall-clock timestamps against a separate log. Every trace record (truncation markers included) now carries `conn: "parent" | "child:<targetId>" | "release:<targetId>"` (#518)
...
- **The per-session proxy log actually explains proxy behavior** — the DAP routing decisions (`MinimalDapClient` dispatch/adoption lines, `ChildSessionManager` outcomes, CDP bridges) were written by module-level loggers into a per-pid `logs/debug-mcp-server-<workerPid>.log` findable only if you knew the worker's pid and that the second file existed at all, while `proxy-<sessionId>.log` — the file the docs and failure payloads point at — showed none of it; during the #513 RCA this made the routing layer look like it didn't log. The worker now re-points those module loggers into the session log at init (per-pid file stays as the pre-init fallback), and DAP response log lines carry `success`/`request_seq` (plus the error message on failures) — whether the adapter *accepted* a request used to be unknowable from any log (#519)
