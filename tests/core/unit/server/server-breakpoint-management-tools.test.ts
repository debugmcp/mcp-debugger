...
describe('Server Breakpoint Management Tools', () => {
  ...
  describe('list_breakpoints', () => {
    ...
    it('says why none of the breakpoints can bind while the launch runs with the debugger off (issue #749)', async () => {
      mockSessionManager.getSession.mockReturnValue({
        id: 'test-session',
        state: 'running',
        sessionLifecycle: 'ACTIVE',
      ...
      const content = JSON.parse(result.content[0].text);
      expect(content.success).toBe(true);
      // The adapter's own per-breakpoint answer is untouched.
      expect(content.breakpoints[0].message).toBe('Unbound breakpoint');
      expect(content.warning).toMatch(/debugger is off for this launch/);
  ...
  describe('remove_breakpoint', () => {
    ...
    it("remove forwards the caller's name even when the policy would rewrite it (issues #550/#559)", async () => {
      // The name is the session layer's business: the handler hands over what
      // the caller typed, never the resolved name — that is what makes a
    ...
    it('offers the package-qualification hint when a bare name with no rewrite matches nothing', async () => {
      mockSessionManager.removeFunctionBreakpointsByName.mockResolvedValue({
        ...
        requestedName: 'compute',
        warning: "for func compute in package main use 'main.compute'"
      });
