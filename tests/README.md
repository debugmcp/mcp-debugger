...
- **`mocks/dap-proxy-doubles.ts`** — `createMockFileSystem()` and `createMockProcessSpawner()` for the proxy worker's own dependency slice, typed to the worker-local `IFileSystem` (a four-member slice of `@debugmcp/shared`'s — a wide double would hide a member the worker starts calling) and `IProcessSpawner` in `src/proxy/dap-proxy-interfaces.ts`; the worker's logger takes `helpers/test-dependencies.ts`'s `createMockLogger()`, since the worker-local `ILogger` is the wider signature. Shared by the proxy-worker and go-initialized-fallback tests
