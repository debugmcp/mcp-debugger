...
import { createTestSessionStore } from '../../utils/test-setup.js';

describe('SessionStore', () => {
  let store: SessionStore;
  let factory: any; // We'll use the factory to demonstrate the pattern

  beforeEach(() => {
    // Using the new test setup utility
    const testSetup = createTestSessionStore();
  ...
  describe('createSession', () => {
    ...
    it('should throw error for unsupported language', () => {
      ...
      expect(() => store.createSession(params)).toThrow(
        "Language 'javascript' is not supported. Only 'python' is currently implemented."
      );
    });
