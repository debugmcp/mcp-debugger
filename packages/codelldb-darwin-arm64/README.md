# @debugmcp/codelldb-darwin-arm64

Prebuilt [CodeLLDB](https://github.com/vadimcn/codelldb) 1.11.8 debug adapter binaries for **macOS arm64 (Apple Silicon)**.
...
The payload is extracted unmodified from the upstream `codelldb-darwin-arm64.vsix` release asset, whose sha256 digest is pinned in the repository's `packages/codelldb-common/vendor-manifest.json`.
