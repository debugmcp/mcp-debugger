# @debugmcp/codelldb-win32-x64

Prebuilt [CodeLLDB](https://github.com/vadimcn/codelldb) 1.11.8 debug adapter binaries for **Windows x64**.
...
The `cpu` field also allows `arm64`: there is no upstream win32-arm64 CodeLLDB build, so Windows-on-ARM installs this x64 package and runs it under x64 emulation.
...
The payload is extracted unmodified from the upstream `codelldb-win32-x64.vsix` release asset, whose sha256 digest is pinned in the repository's `packages/codelldb-common/vendor-manifest.json`.
